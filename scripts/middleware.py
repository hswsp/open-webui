...
async def process_chat_payload(request, form_data, user, metadata, model):
    ...
    if chat_id and user_message_id and not chat_id.startswith('local:') and not chat_id.startswith('channel:'):
        ...
        if db_messages:
            ...
            # Only inject image files for vision-capable models
            model_caps = model.get('info', {}).get('meta', {}).get('capabilities', {})
            model_supports_vision = model_caps.get('vision', False)
            for message in form_data['messages']:
                ...
                if message.get('role') == 'user' and image_files:
                    ...
                    if model_supports_vision:
                        ...
                        # Add <file> references so non-vision models know about images
                        file_refs = ' '.join(
                            f'<file type="image" uuid="{f.get("url","")}"/>'
                            for f in image_files if f.get('url')
                        )
                        if file_refs and isinstance(text_content, str):
                            message['content'] = f'{text_content} {file_refs}'.strip()
    ...
    # Strip image_url parts for non-vision models before base64 conversion
    model_caps = model.get('info', {}).get('meta', {}).get('capabilities', {})
    if not model_caps.get('vision', False):
        for message in form_data['messages']:
            content = message.get('content')
            if isinstance(content, list):
                text_parts = [item for item in content if not (isinstance(item, dict) and item.get('type') == 'image_url')]
                if text_parts:
                    combined = ''.join(p.get('text', '') for p in text_parts if isinstance(p, dict))
                    message['content'] = combined if combined else content
                else:
